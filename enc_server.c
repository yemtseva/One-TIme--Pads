...
char* encryptMessage(char* message, char* key, int textLength){
	...
	for(i = 0; i < strlen(message); i++){
		...
		if(ciphertext[i] > MOD + 65) // have to add 65 since A is 65 on ASCII table
...
int main(int argc, char *argv[]){
	...
	// Accept the connection request which creates a connection socket
		...
		if(pid == 0){
			...
			while(i < 3){
				...
				if(i == 0){
					if(strcmp(buffer, "enc_client") != 0){
						fprintf(stderr,"dec_client tried to connect to enc_server - NOT POSSIBLE", argv[3]); 
						exit(2);
					}
				}
				else if(i == 1)
			...
			ciphertext = encryptMessage(plaintext, key, strlen(plaintext));
